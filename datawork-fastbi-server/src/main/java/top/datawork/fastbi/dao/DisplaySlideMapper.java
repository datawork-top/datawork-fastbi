...
import top.datawork.fastbi.dto.displayDto.SlideWithDisplayAndProject;
...
public interface DisplaySlideMapper {
    ...
    @Delete({"delete from fastbi_display_slide where id = #{id}"})
    int deleteById(@Param("id") Long id);

    @Delete({"DELETE FROM fastbi_display_slide " +
            " where display_id in (SELECT id from fastbi_display WHERE project_id = #{projectId})"})
    int deleteByProjectId(@Param("projectId") Long projectId);

    @Select({"select * from fastbi_display_slide where id = #{id}"})
    DisplaySlide getById(Long id);

    @Update({
            "update fastbi_display_slide",
    ...
    @Select({"select * from fastbi_display_slide where display_id = #{displayId} order by `index`"})
    List<DisplaySlide> selectByDisplayId(@Param("displayId") Long displayId);

    @Delete({"delete from fastbi_display_slide where display_id = #{displayId}"})
    ...
    @Select({
            ...
            "FROM fastbi_display_slide s ",
            "   LEFT JOIN fastbi_display d on d.id = s.display_id",
            "   LEFT JOIN fastbi_project p on p.id = d.project_id",
            "where s.id = #{slideId}",
    })
