...
import top.datawork.fastbi.dto.displayDto.MemDisplaySlideWidgetWithSlide;
...
public interface MemDisplaySlideWidgetMapper {
    ...
    @Delete({"delete from fastbi_mem_display_slide_widget where id = #{id}"})
    int deleteById(@Param("id") Long id);

    int deleteBatchById(@Param("list") List<Long> list);

    @Delete({
            "delete from fastbi_mem_display_slide_widget where display_slide_id in ",
            "(SELECT s.id FROM fastbi_display_slide s LEFT JOIN display d on s.display_id = d.id where d.project_id = #{projectId})"
    })
    int deleteByProject(@Param("projectId") Long projectId);

    @Select({"select * from fastbi_mem_display_slide_widget where id = #{id}"})
    ...
    @Select({"SELECT m.* FROM fastbi_mem_display_slide_widget m WHERE m.display_slide_id = #{slideId}"})
    List<MemDisplaySlideWidget> getMemDisplaySlideWidgetListBySlideId(@Param("slideId") Long slideId);

    @Delete({"delete from fastbi_mem_display_slide_widget where display_slide_id in (select id from fastbi_display_slide where display_id = #{displayId})"})
    int deleteByDisplayId(@Param("displayId") Long displayId);


    @Delete({"delete from fastbi_mem_display_slide_widget where display_slide_id = #{slideId}"})
    ...
    @Select({
            ...
            "FROM fastbi_mem_display_slide_widget m LEFT JOIN fastbi_display_slide s on m.display_slide_id = s.id",
    ...
    @Delete({"delete from fastbi_mem_display_slide_widget where widget_id = #{widgetId}"})
