...
import top.datawork.fastbi.dto.dashboardDto.DashboardTree;
...
import top.datawork.fastbi.service.screenshot.ScreenshotUtil;
import top.datawork.fastbi.service.share.ShareDataPermission;
import top.datawork.fastbi.service.share.ShareFactor;
import top.datawork.fastbi.service.share.ShareMode;
    ...
    protected List<CronJobContent> getCronJobContents(CronJobConfig cronJobConfig, Map<String, Integer> orderMap,
        ...
        for (int i = 0; i < cronJobConfig.getContentList().size(); i++) {
            ...
            if (cronJobContent.getContentType().equalsIgnoreCase(DISPLAY)) {
                ...
                orderWeightMap.put(DISPLAY + Consts.AT_SYMBOL + cronJobContent.getId(), orderWeight);
                jobContentList.add(cronJobContent);
                orderMap.put(DISPLAY + Consts.AT_SYMBOL + cronJobContent.getId(), orderWeight);
            } else {
                orderWeightMap.put(PORTAL + Consts.AT_SYMBOL + cronJobContent.getId(), orderWeight);
                if (CollectionUtils.isEmpty(cronJobContent.getItems())) {
            ...
            portalDashboardsMap.forEach((pId, ds) -> {
                ...
                if (!CollectionUtils.isEmpty(list)) {
                    for (int i = 0; i < list.size(); i++) {
                        ...
                        if (!orderMap.containsKey(DASHBOARD + Consts.AT_SYMBOL + node.getId())) {
                            orderMap.put(DASHBOARD + Consts.AT_SYMBOL + node.getId(), i + orderWeightMap.get(PORTAL + Consts.AT_SYMBOL + pId));
                        }
                    }
                }
            });
