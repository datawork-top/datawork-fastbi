...
import top.datawork.fastbi.core.model.SqlFilter;
import top.datawork.fastbi.core.utils.SqlParseUtils;
import top.datawork.fastbi.dao.RelRoleViewMapper;
import top.datawork.fastbi.dao.SourceMapper;
...
import top.datawork.core.utils.*;
import top.datawork.fastbi.core.enums.*;
...
ViewServiceImpl extends BaseEntityService implements ViewService {
            ...
            if (!CollectionUtils.isEmpty(sqlEntity.getQueryParams())) {
                sqlEntity.getQueryParams().forEach((k, v) -> {
                    if (v instanceof List && ((List) v).size() > 0) {
                        v = ((List) v).stream().collect(Collectors.joining(Consts.COMMA)).toString();
                    }
        ...
        try {
            ...
            if (!CollectionUtils.isEmpty(querySqlList)) {
                ...
                if (null != executeParam.getCache() && executeParam.getCache() && executeParam.getExpired() > 0L) {
                    ...
                    saltBuilder.append(Consts.MINUS);
                ...
                authVariables.forEach(sqlVariable -> {
                    futures.add(executorService.submit(() -> {
                        if (null != sqlVariable) {
                            ...
                            if (null == values) {
                                vSet.add(Constants.NO_AUTH_PERMISSION);
    ...
    private List<SqlVariable> getQueryVariables(List<SqlVariable> variables) {
        if (!CollectionUtils.isEmpty(variables)) {
            return variables.stream().filter(v -> SqlVariableTypeEnum.QUERYVAR == SqlVariableTypeEnum.typeOf(v.getType())).collect(Collectors.toList());
        }
        return null;
    }
            ...
            if (!StringUtils.isEmpty(r.getRowAuth())) {
                ...
                authVarMap.forEach((k, v) -> {
                        ...
                        optional.ifPresent(p -> {
                            if (p.isEnable()) {
                                if (CollectionUtils.isEmpty(p.getValues())) {
                                    sqlVariable.setDefaultValues(Arrays.asList(new String[]{Constants.NO_AUTH_PERMISSION}));
                                } else {
                    ...
                    if (optional.isPresent()) {
                        ...
                        if (p.isEnable()) {
                            if (!CollectionUtils.isEmpty(p.getValues())) {
                                boolean denied = defaultValues.size() == 1 && defaultValues.get(0).equals(Constants.NO_AUTH_PERMISSION);
